...
- **Dependencies:** `tower-http` with the `trace` feature. Already present in `Cargo.lock` at 0.6.11 via reqwest, but compiled without `trace` — so this adds a `Cargo.toml` entry and a feature-unification rebuild, and **zero new crates** to the tree (verified). The JSON log format additionally needs `tracing-subscriber`'s `json` feature, which does pull in one new crate, `tracing-serde`.
