...
**5. Extend the schema transform to normalize optional-enum schemas, with `#[schemars(inline)]` on the new enums.**
Implementation finding: with `#[schemars(inline)]`, schemars merges `Option<UnitEnum>` to `"type": ["string", "null"]` with a `null` inside the `enum` value list — no `anyOf` at all. The load-bearing fix is therefore in `collapse_nullable_types`: when the null branch is dropped from a type array, `null` is also dropped from a sibling `enum` list. An `anyOf`-with-null collapse (replace `anyOf: [X, {"type": "null"}]` with `X`, merging sibling keys like `description`) is implemented alongside as defense for non-inlined optional composites, and the regression test's negative control covers the `anyOf` shape.
Scope note: enums whose variants carry doc comments (`ImageType`) derive as a `oneOf` of documented `const` variants and are left as `$defs`/`$ref` — the per-variant descriptions are worth more to model callers than inlining, and those parameters are required, so no nullable wrapper arises. The strictness invariant therefore forbids `anyOf` but permits `$ref` for nested/object subschemas.
