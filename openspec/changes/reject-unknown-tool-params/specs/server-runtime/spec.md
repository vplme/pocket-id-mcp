...
Tool input handling SHALL be closed-world. For every tool that declares parameters, a call whose arguments contain a top-level key not declared in the tool's input schema SHALL be rejected as a JSON-RPC invalid-params error (`-32602`) naming the unknown field, and the handler SHALL NOT run. (Parameterless tools advertise the same strictness via their schema; stray arguments there have no parameter to misdirect and are ignored by the serving SDK.) Every advertised tool input schema SHALL declare `additionalProperties: false` at its top level, matching the enforced behavior. Parameters with a closed set of valid values known at build time (sort direction; audit-log location) SHALL be advertised as string enums in the input schema and SHALL reject values outside the set with an error naming the valid values; optional enum parameters SHALL be advertised as a plain inline enum schema (no `anyOf`/`$ref` indirection), consistent with the plain-types requirement for optional parameters.
